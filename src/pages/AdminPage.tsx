...
export default function AdminPage() {
  ...
  return <main className="admin-page">
    ...
    <section className="admin-main">
      ...
      {tab === 'media' && <section className="admin-panel"><div className="admin-panel__head"><div><h2>Media library</h2><p>Images, diagrams, videos, and résumé documents.</p></div><form className="admin-upload" onSubmit={uploadMedia}><input type="file" name="file" required /><input name="alt_text" placeholder="Alt text" /><button>Upload</button></form></div><div className="admin-media-grid">{media.map((item) => <article key={String(item.id)}>{String(item.mime_type).startsWith('image/') ? <img src={String(item.public_url)} alt={String(item.alt_text || '')} /> : <div>{String(item.kind)}</div>}<strong>{String(item.filename)}</strong><span>{Math.round(Number(item.size_bytes)/1024)} KB</span><button onClick={() => remove('media', item)}>Delete</button></article>)}</div></section>}
