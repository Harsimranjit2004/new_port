...
function JsonEditor({ value, onChange, label }: { value: unknown; onChange: (value: RecordValue) => void; label: string }) {
  const [text, setText] = useState(() => JSON.stringify(value, null, 2))
  const [error, setError] = useState('')
  useEffect(() => { setText(JSON.stringify(value, null, 2)); setError('') }, [value])
  const update = (next: string) => {
    setText(next)
    try { onChange(JSON.parse(next)); setError('') } catch { setError('Invalid JSON') }
  }
