...
export default function About2Page() {
  ...
  return <div id="top" className="a2 white-surface"><Navbar submergedAt={Number.POSITIVE_INFINITY} /><main><Observer name={profile?.name ?? text(introContent.title) ?? intro.name} observer={text(introContent.observer)} paragraphs={paragraphs.length ? paragraphs : intro.paragraphs} links={dynamicLinks.length ? dynamicLinks : intro.links} portraitUrl={profile?.avatar_url} portraitAlt={profile?.avatar_alt ?? `Portrait of ${profile?.name ?? intro.name}`} /><Work experienceItems={experienceItems} domains={domains} technologyItems={technologyItems.length ? technologyItems : technologies} /><Shelf items={shelfItems} /><Outside images={outsideImages} /></main><Footer /></div>
}
