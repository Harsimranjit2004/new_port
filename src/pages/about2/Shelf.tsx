...
export default function Shelf() {
  ...
  return <section className="a2-shelf" aria-labelledby="a2-shelf-heading"><div className="a2-shell"><div className="a2-shelf__intro"><header><p className="a2-eyebrow">03 / The shelf</p><h2 id="a2-shelf-heading">Books that shape how I think<span>.</span></h2><p>Ideas from engineers, writers, and thinkers<br />I keep coming back to.</p></header><aside className="a2-current-book" aria-label="Currently reading"><p>Currently reading</p><div><div className="a2-current-book__cover"><Cover book={currentBook} decorative /></div><section><h3>{currentBook.title}</h3><strong>{currentBook.author}</strong><i /><p>{currentBook.note}</p></section></div></aside></div><div className="a2-shelf-installation">{shelfGroups.map((group) => <div className="a2-shelf-level" key={group.id}><div className="a2-shelf-level__books">{books.filter((book) => book.shelf === group.id).map((book) => <ShelfBook book={book} key={book.title} />)}</div><div className="a2-shelf-plank" aria-hidden="true" /></div>)}</div></div></section>
}
